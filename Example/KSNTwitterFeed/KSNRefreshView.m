...
- (UINib *)nib
{
    ...
    {
        return [UINib nibWithNibName:[NSStringFromClass([self class]) stringByAppendingString:@"Vertical"] bundle:bundle];
    }
    else
    {
...
- (void)loadXibContent
{
    ...
    self.loadingIndicator = [[KSNLoadingIndicator alloc] init];
...
- (void)setRefreshing:(BOOL)refreshing
{
    if (_refreshing != refreshing)
    {
        ...
        if (refreshing)
        {
            [self.loadingIndicator fakeProgressWithDuration:1];
        }
        else
        {
